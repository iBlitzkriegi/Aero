            ...
            if(args[0].equalsIgnoreCase("share")){
                if(!Mute.muted.contains(message.getAuthor().getId())) {
                    if(args.length == 2){
                        ...
                        builder.append(Settings.getMsgStart() + "Here you go " + message.getAuthor().getMentionTag() + "! Just type `" + Settings.getCommandStart() + "share ImgName` to get one of these images!").appendNewLine().append("```xl").appendNewLine().append("{*}---=CurrentImages=---{*}").appendNewLine();
